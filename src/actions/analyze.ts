...
export async function analyzeBookmark(url: string, title?: string): Promise<AnalysisResult | null> {
    ...
    `;
        ...
        return {
            ...
            status: ['saved', 'reading', 'completed'].includes(data.status) ? data.status : 'saved',
        };
    } catch (error) {
